...
if _config.logging.console.enable:
    logger.add(
        sink=sys.stdout,
        level=_config.logging.console.level,
        format=log_format,
    )

# 根据配置决定是否写入文件日志，并在启动时确保日志目录存在
if _config.logging.file.enable:
    path = Path(_config.logging.file.path)
    path.mkdir(parents=True, exist_ok=True)
    logger.add(
        sink=path / "app.log",
        level=_config.logging.file.level,
        format=log_format,
        rotation=_config.logging.file.rotation,
        retention=_config.logging.file.retention,
        encoding="utf-8",
    )
