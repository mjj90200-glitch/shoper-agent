...
> **实施状态更新（2026-09-02）**：Task 1–7 已完成，并新增了 `tests/test_memory.py` 自动化覆盖。实现按产品决策做了两处调整：`rewrite_query` 只读取历史，成功的数据问数才在 `run_sql` 写入用户问题与结果摘要；所有非数据问题统一固定回复，且不写入 `messages`。Task 8 的真实 Docker + LLM 端到端验证待本机 Docker 环境可用后执行。
